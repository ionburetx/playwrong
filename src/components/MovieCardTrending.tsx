    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute inset-0 flex items-center md:items-start bg-gradient-to-r from-black to-transparent">
        <div className="p-4 ml-4 md:p-8 md:pl-0 md:ml-16 max-w-2xl md:mt-[35vh]">
          ...
          {isTrending && (
            <p className="text-white text-sm md:text-lg mb-6 ml-4 mr-4 md:mx-4 opacity-0 animate-[fadeIn_1s_ease-out_0.7s_forwards] line-clamp-4 md:line-clamp-none">{movie.overview}</p>
          )}
          <Link
            to={`/movie/${movie.id}`}
            className="inline-block px-6 py-3 bg-blue-600 text-white rounded-md 
              hover:bg-blue-700 transition-colors ml-4 md:ml-4 mb-12 opacity-0 animate-[fadeIn_1s_ease-out_0.7s_forwards]"
          >
            Ver más
          </Link>
        </div>
